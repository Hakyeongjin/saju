...
// 일간별 "이런 사람" 예시 (쉬운 말)
export const DAY_MASTER_EXAMPLE: Record<string, string> = {
  甲: '모임에서 자연스럽게 리더를 맡고, 큰 목표를 향해 앞장서 팀을 이끄는 스타일이에요.',
  乙: '낯선 환경에서도 금세 적응하고, 사람들 사이에서 부드럽게 처신하며 실속을 챙겨요.',
  丙: '어디서든 분위기 메이커, 감정이 얼굴에 그대로 드러나는 솔직하고 밝은 사람이에요.',
  丁: '좋아하는 한 가지에 밤새 몰입하는 집중형인데, 기분에 따라 컨디션이 오르내려요.',
  戊: '친구들이 고민을 털어놓는, 듬직한 맏형·맏언니 같은 사람이에요.',
  己: '조용히 주변을 챙기고 살림·정리를 야무지게 해내는 실속형이에요.',
  庚: '옳고 그름이 분명하고, 한번 결정하면 바로 밀어붙이는 화끈한 사람이에요.',
  辛: '옷차림·마무리가 깔끔하고, 작은 실수도 오래 신경 쓰는 완벽주의예요.',
  壬: '관심사가 넓어 이것저것 벌이지만, 가끔 마무리가 약해지는 자유로운 사람이에요.',
  癸: '남의 마음을 섬세하게 읽는 사람인데, 생각이 많아 걱정을 오래 붙들곤 해요.',
}
...
// 십신별 "쉽게 말하면 + 예시"
export const SHISHEN_EXAMPLE: Record<string, string> = {
  비견: '쉽게 말하면 「내 편이자 라이벌」. 형제·동창처럼 비슷한 사람들과 경쟁하며 크는 스타일.',
  겁재: '쉽게 말하면 「승부욕 강한 경쟁자」. 친구와 동업했다가 돈 문제로 부딪히기 쉬운 타입.',
  식신: '쉽게 말하면 「즐기며 만들어내는 재주」. 요리·글쓰기처럼 좋아하는 걸 꾸준히 해서 인정받는 스타일.',
  상관: '쉽게 말하면 「톡톡 튀는 끼와 말솜씨」. 유튜버·강사처럼 표현으로 승부하지만 규칙은 답답해함.',
  편재: '쉽게 말하면 「크게 굴리는 사업 감각」. 장사·투자로 큰돈을 벌지만 씀씀이도 큰 스타일.',
  정재: '쉽게 말하면 「알뜰히 모으는 감각」. 착실히 저축해 안정적으로 재산을 쌓아가는 타입.',
  편관: '쉽게 말하면 「카리스마 있는 리더」. 군인·경찰·현장 지휘처럼 강하게 이끄는 자리에 어울림.',
  정관: '쉽게 말하면 「반듯한 모범생」. 공무원·대기업처럼 규칙과 책임을 지키는 조직에 잘 맞음.',
  편인: '쉽게 말하면 「눈치 빠른 아이디어뱅크」. 기획·연구·전문기술처럼 남다른 시각이 필요한 일에 강함.',
  정인: '쉽게 말하면 「배움과 어른 복」. 공부·자격증으로 실력을 쌓고 윗사람 도움을 잘 받는 스타일.',
}
...
export interface ShishenAnalysis {
  ...
  present: { name: string; group: ShishenGroup; count: number; text: string; example: string }[]
  summary: string
}
...
function analyzeShishen(result: SajuResult): ShishenAnalysis {
  ...
  const present = [...nameCount.entries()]
    ...
    .map(([name, count]) => ({
      ...
      example: `${count >= 2 ? '내 사주에 여러 개라 이 기질이 강하게 나타나요. ' : ''}${SHISHEN_EXAMPLE[name]}`,
    }))
...
export function interpret(result: SajuResult): Interpretation {
  ...
  const ELEMENT_TRAIT: Record<Element, string> = {
    목: '성장·추진력의 기운 — 많으면 밀어붙이는 힘이 세고, 없으면 시작이 더딜 수 있어요',
    화: '열정·표현의 기운 — 많으면 활발하지만 욱하기 쉽고, 없으면 감정 표현이 서툴 수 있어요',
    토: '안정·끈기의 기운 — 많으면 듬직하지만 고집스럽고, 없으면 중심이 흔들리기 쉬워요',
    금: '결단·원칙의 기운 — 많으면 칼 같지만 날카롭고, 없으면 맺고 끊음이 약할 수 있어요',
    수: '지혜·융통성의 기운 — 많으면 유연하지만 변덕스럽고, 없으면 임기응변이 아쉬울 수 있어요',
  }
